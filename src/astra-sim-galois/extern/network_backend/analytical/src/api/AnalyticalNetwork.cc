...
int AnalyticalNetwork::sim_send(
  ...
  // std::cout << "Sim_Send function is called for src: " << src << " and dest: " << dst << "; the chunk num is: " << tag << std::endl; // Galois debug print
  ...
  // std::cout << "AnalyticalNetwork::sim_send: " << "src = " << src << ", dest = " << dst << " (dimension = " << used_dim << "), count = " << count << ", delta.time_val = " << delta.time_val << ", send_start_time = " << sim_get_time().time_val << " ns." << " The chunk ID = " << this->stream_num_ID << ", and the one-hop communication ID = " << this->stream_count_ID << ", and chunk stage = " << this->chunk_stage << std::endl; // Galois useful debug print which can see communication size
