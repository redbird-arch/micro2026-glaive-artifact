...
class Topology {
 public:
  ...
  virtual std::pair<double, int> send(
      ...
      int chunk_stage,
      int failure_type) noexcept = 0; // currently we only change send function
 ...
 protected:
  ...
  Latency serializationLatency(int dimension, PayloadSize payload_size, int acceleratation_element)
      const noexcept;
