...
Topology::Latency Topology::serializationLatency(
    ...
    int acceleratation_element) const noexcept {
      // printf("For serialization delay, payload_size = %lu, and bandwidth = %f\n", static_cast<unsigned long>(payload_size), configs[dimension].getLinkBandwidth()); // Galois deug print: actually the link_bandwidth should multiply 1.024^3 due to it should convert from GB/s to B/s
