...
Sys::Sys(
  ...
  this->failure_dim = 0;
  this->failure_type = 0; // give a value here
  this->current_layer_collective_type = ComType::All_Reduce; // initilization
  this->external_collective_file = "";
...
int Sys::break_dimension(int model_parallel_npu_group) {
  ...
  int all_npus = 1;
  // physical_dims is just units-count list in network input file (e.g., [8,8] in TPUv3)
  for (; dimension_to_break < physical_dims.size(); dimension_to_break++) {
    // break until the nodes number could cover (bigger than) model_parallel_npu_group
    if (all_npus * physical_dims[dimension_to_break] <
      ...
      all_npus *= physical_dims[dimension_to_break];
        ...
        all_npus * physical_dims[dimension_to_break] >
        model_parallel_npu_group) {
      // start to break and delete existing logical topo
      for (auto lt : logical_topologies) {
        delete lt.second;
      }
      logical_topologies.clear();
      ...
      scheduler_unit = new SchedulerUnit(
          ...
          concurrent_streams);
      vLevels = new QueueLevels(queues_per_dim, 0, NI->get_backend_type());
      // start to rebuild the logical topo (e.g., [8,8] tpuv3 meets tp=16, then [8,2] as a tp group and 4 groups exist)
      ...
      // update collective implementation (add a dim) for each collective
      ...
      std::cout << "Break_Dimension function: logical dims = "<< logical_dims.size() << " (e.g., [";
...
int Sys::sim_send(
  ...
  if (NI->collective_type == ComType::All_to_All) {
    ...
    link_failure_scheduling_flag_temp = 2; // default MATEe for all other collectives
  }
...
bool Sys::parse_var(std::string var, std::string value) {
  ...
  if (var == "scheduling-policy:") {
    ...
    // get failure_dim, we only support failure in one dim currently
    ...
    // here are five failure types for two-link-failures:
  ...
  } else if (var == "external-collective-file:") {
    std::stringstream mval(value);
    mval >> external_collective_file;
  } else if (var != "") {
    std::cerr
        << "######### Exiting because " << var
...
bool Sys::post_process_inputs() {
  ...
  if (!external_collective_file.empty()) {
    load_external_collective_file(external_collective_file);
  }
  return true;
}
void Sys::load_external_collective_file(const std::string& path) {
  std::ifstream in(path);
  if (!in) {
    sys_panic("unable to open external collective file: " + path);
  }

  external_collective_delays.clear();
  std::string line;
  int loaded_rows = 0;
  while (std::getline(in, line)) {
    ...
    std::string phase = fields[1];
    if (phase == "phase") {
      continue;
    }
    if (phase != "fwd" && phase != "ig" && phase != "wg") {
      sys_panic(
          "unknown phase in external collective file " + path + ": " + phase);
    }

    std::string layer_key = fields[0];
    if (layer_key.empty()) {
      continue;
    }
    double duration_us = std::stod(fields[2]);
    Tick delay_cycles = static_cast<Tick>(std::llround(duration_us * FREQ));
    external_collective_delays[{layer_key, phase}] = delay_cycles;
    loaded_rows++;
  }

  if (id == 0) {
    std::cout << "Loaded " << loaded_rows
              << " external collective delay rows from " << path << std::endl;
  }
}
Tick Sys::lookup_external_collective_delay(
    const std::string& layer_id,
    int layer_num,
    const std::string& phase) const {
  auto it = external_collective_delays.find({layer_id, phase});
  if (it == external_collective_delays.end()) {
    if (layer_num >= 0) {
      const std::string one_based = std::to_string(layer_num + 1);
      auto one_based_it = external_collective_delays.find({one_based, phase});
      if (one_based_it != external_collective_delays.end()) {
        return one_based_it->second;
      }

      const std::string zero_based = std::to_string(layer_num);
      auto zero_based_it = external_collective_delays.find({zero_based, phase});
      if (zero_based_it != external_collective_delays.end()) {
        return zero_based_it->second;
      }
    }
    return 0;
  }
...
DataSet* Sys::generate_collective(
  ...
  while (size > 0) {
    ...
    } else if ((collective_type == ComType::All_to_All) && (link_failure_scheduling == LinkFailureScheduling::Mate || link_failure_scheduling == LinkFailureScheduling::Mate_Enhanced) && (inter_dimension_scheduling == InterDimensionScheduling::ND_Torus_Ring || inter_dimension_scheduling == InterDimensionScheduling::ND_Torus_Ring_AlltoAll_AllReduce) && (failure_type == 1 || failure_type == 4)) {
    ...
    if (inter_dimension_scheduling == InterDimensionScheduling::RoundRobin) {
      ...
      inter_dimension_scheduling == 
           InterDimensionScheduling::ND_Torus_Ring_AlltoAll_AllReduce)){
      dim_mapper = nd_torus_ring->get_chunk_scheduling(stream_counter, link_failure_scheduling, failure_type); // a vector for different chunks, showing the execution sequence of dimensions with fixed chunkID 
    }
    ...
    if (collective_type != ComType::All_Reduce ||
      ...
      if (link_failure_scheduling == LinkFailureScheduling::Mate || link_failure_scheduling == LinkFailureScheduling::Mate_Enhanced) {
        for (int dim = 0; dim < dim_mapper.size(); dim++) { //           this->chunk_stage = dim;
          NI->chunk_stage = this->chunk_stage;
          ...
          std::cout << "Execution dimension: " << queue.first << ", and direction(clockwise: 0, anti: 1): " << static_cast<int>(queue.second) << " for chunk " << stream_counter << " and stage " << dim << std::endl; 
      ...
      // for CollectiveOptimization == localBWAware scenario 
      // the all reduce can be divided as reduce-scatter from dim 0 to N-1, all-reduce at dim N, all-gather from dim N-1 to dim 0
      int dim = 0;
    ...
    if (vect.size() > 0) {
      // Here each collective phase is represented by a stream
      std::cout << "[DEBUG] chunk " << stream_counter 
          << " has " << vect.size() << " phases\n";
...
void Sys::try_register_event(
  ...
  if (should_schedule) {
    // tranfer the time from Tick to timespec_t
