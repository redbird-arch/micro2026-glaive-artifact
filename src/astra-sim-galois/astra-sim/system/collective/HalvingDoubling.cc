...
HalvingDoubling::HalvingDoubling(
  ...
  switch (type) {
    ...
    default:
      std::cerr
          << "######### Exiting because of unknown communication type for HalvingDoubling collective algorithm #########" // They don't support All-to-All for Halving Doubling Algorithm
          << std::endl;
      std::exit(1);
  }
  RingTopology::Direction direction = specify_direction();
  this->current_receiver = id;
  for (int i = 0; i < rank_offset; i++) { // update one-by-one in a ring style, until the sender and receiver are exactly what the rank-offset indicates
...
int HalvingDoubling::get_non_zero_latency_packets() {
  return log2(nodes_in_ring) - 1 * parallel_reduce;  // any problem?
}
