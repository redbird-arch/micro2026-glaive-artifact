...
namespace AstraSim {
namespace {
bool maybe_issue_external_collective_delay(
    Layer* layer,
    Tick delay,
    std::map<int, DataSet*>& datasets,
    EventType finish_event,
  ...
  datasets[dataset->my_id] = dataset;
  dataset->set_notifier(layer, finish_event);
  layer->generator->register_event(dataset, EventType::General, nullptr, delay);
  if (layer->generator->id == 0) {
    std::cout << "At Time " << Sys::boostedTick()
              << ", info: external " << phase_name
              << " collective delay injected for layer: " << layer->id
              << " , delay cycles = " << delay << std::endl;
  }
  return true;
}
} // namespace
Layer::Layer(
    ...
    uint64_t fwd_pass_mem_read_bytes,
    uint64_t fwd_pass_mem_write_bytes,
  ...
  this->fwd_pass_mem_read_bytes = fwd_pass_mem_read_bytes;
  this->fwd_pass_mem_write_bytes = fwd_pass_mem_write_bytes;
  ...
  this->total_forward_pass_memory = 0;
...
Tick Layer::get_fwd_pass_memory() {
  Tick memory_delay = 0;
  if (fwd_pass_mem_read_bytes > 0) {
    memory_delay += generator->mem_read(fwd_pass_mem_read_bytes);
  }
  if (fwd_pass_mem_write_bytes > 0) {
    memory_delay += generator->mem_write(fwd_pass_mem_write_bytes);
  }
  total_forward_pass_memory += memory_delay;
  return memory_delay;
}
...
LayerData Layer::report(
  ...
  total_memory += (total_forward_pass_memory / FREQ);
  ...
  layerData.total_forward_pass_memory = total_forward_pass_memory / FREQ;
  ...
  if (seprate_log) {
    ...
    std::cout << "id: " << id << " ,Total cycles spent on fwd pass memory: "
              << total_forward_pass_memory << std::endl;
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 3, "fwd memory");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        3,
        std::to_string(total_forward_pass_memory / FREQ));

    std::cout << "id: " << id << " ,Total cycles spent on weight grad compute: "
              << total_weight_grad_compute << std::endl;
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 4, "wg compute");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        4,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 5, "ig compute");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        5,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 6, "fwd exposed comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        6,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 7, "wg exposed comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        7,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 8, "ig exposed comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        8,
    ...
    std::cout << "id: " << id
              ...
              << std::endl;
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 9, "fwd total comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        9,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 10, "wg total comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        10,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 11, "ig total comm");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        11,
    ...
    if (stat_row == 0 && layer_num == 0) {
      EndToEnd->write_cell(0, 12, "workload finished at");
    }
    EndToEnd->write_cell(
        layer_num * total_rows + 1 + stat_row,
        12,
    ...
    if (layer_num == workload->SIZE - 1) {
      if (stat_row == 0) {
        EndToEnd->write_cell(0, 13, "total comp");
        EndToEnd->write_cell(0, 14, "total memory");
        EndToEnd->write_cell(0, 15, "total exposed comm");
      }
      total_exposed =
          (((double)Sys::boostedTick()) / FREQ) - total_compute - total_memory;
      EndToEnd->write_cell(1 + stat_row, 13, std::to_string(total_compute));
      EndToEnd->write_cell(1 + stat_row, 14, std::to_string(total_memory));
      EndToEnd->write_cell(1 + stat_row, 15, std::to_string(total_exposed));
    }
...
void Layer::issue_forward_pass_comm(
  ...
  collective_counter++;
  Tick external_delay =
      generator->lookup_external_collective_delay(id, layer_num, "fwd");
  if (maybe_issue_external_collective_delay(
          this,
          external_delay,
          fwd_pass_datasets,
  ...
  if (fwd_pass_comm_type == ComType::All_to_All_V) {
    Sys::sys_panic(
        "ALLTOALLV requires external collective delay for layer: " + id);
  }
  if (fwd_pass_comm_type == ComType::All_Reduce) {
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = FWD All-Reduce, Size = " << fwd_pass_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < fwd_pass_comm_involved_dimensions.size(); i++) {
      std::cout << fwd_pass_comm_involved_dimensions[i] << ", ";
    }
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = FWD All-to-All, Size = " << fwd_pass_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < fwd_pass_comm_involved_dimensions.size(); i++) {
      std::cout << fwd_pass_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = FWD All-Gather, Size = " << fwd_pass_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < fwd_pass_comm_involved_dimensions.size(); i++) {
      std::cout << fwd_pass_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = FWD Reduce-Scatter, Size = " << fwd_pass_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < fwd_pass_comm_involved_dimensions.size(); i++) {
      std::cout << fwd_pass_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
...
void Layer::issue_input_grad_comm(
  ...
  collective_counter++;
  Tick external_delay =
      generator->lookup_external_collective_delay(id, layer_num, "ig");
  if (maybe_issue_external_collective_delay(
          this,
          external_delay,
          input_grad_datasets,
  ...
  if (input_grad_comm_type == ComType::All_to_All_V) {
    Sys::sys_panic(
        "ALLTOALLV requires external collective delay for layer: " + id);
  }
  if (input_grad_comm_type == ComType::All_Reduce) {
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_IG All-Reduce, Size = " << input_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < input_grad_comm_involved_dimensions.size(); i++) {
      std::cout << input_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_IG All-to-All, Size = " << input_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < input_grad_comm_involved_dimensions.size(); i++) {
      std::cout << input_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_IG All-Gather, Size = " << input_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < input_grad_comm_involved_dimensions.size(); i++) {
      std::cout << input_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_IG Reduce-Scatter, Size = " << input_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < input_grad_comm_involved_dimensions.size(); i++) {
      std::cout << input_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
...
void Layer::issue_weight_grad_comm(
  ...
  collective_counter++;
  Tick external_delay =
      generator->lookup_external_collective_delay(id, layer_num, "wg");
  if (maybe_issue_external_collective_delay(
          this,
          external_delay,
          weight_grad_datasets,
          EventType::Wight_Grad_Comm_Finished,
          "weight-gradient")) {
    return;
  }
  if (weight_grad_comm_type == ComType::All_to_All_V) {
    Sys::sys_panic(
        "ALLTOALLV requires external collective delay for layer: " + id);
  }
  if (weight_grad_comm_type == ComType::All_Reduce) {
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_WG All-Reduce, Size = " << weight_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < weight_grad_comm_involved_dimensions.size(); i++) {
      std::cout << weight_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_WG All-to-All, Size = " << weight_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < weight_grad_comm_involved_dimensions.size(); i++) {
      std::cout << weight_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_WG All-Gather, Size = " << weight_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < weight_grad_comm_involved_dimensions.size(); i++) {
      std::cout << weight_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
    ...
    // print comm related info here
    std::cout << "At Time " << Sys::boostedTick() << ", Type = BWD_WG Reduce-Scatter, Size = " << weight_grad_comm_size << ", Involved_Dim = [";
    for (int i = 0; i < weight_grad_comm_involved_dimensions.size(); i++) {
      std::cout << weight_grad_comm_involved_dimensions[i] << ", ";
    }
    std::cout << "], Layer_Num = " << layer_num << std::endl;
