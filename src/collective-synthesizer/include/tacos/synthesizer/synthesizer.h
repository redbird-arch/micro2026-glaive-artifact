...
class Synthesizer {
  public:
    ...
    [[nodiscard]] Time solve(const std::shared_ptr<Topology> topology,
                             const std::shared_ptr<Collective> collective,
                             ChunkSize chunkSize) noexcept;
    
    /// support multi-thread
    static void setThreadOutputStream(std::ostream& os);
  ...
  private:
    ...
    /// @brief Temporary routing requirements for intermediate NPUs
    PostconditionMap tempConditions_ = {};

    /// @brief Hop-based diameter of the current topology
    int diameter_ = 0;

    /// @brief Cached node degrees for current topology
    std::vector<int> nodeDegrees_;

    /// @brief Maximum number of reschedule attempts per timestep
    int maxRescheduleAttempts_ = 10;

    /// @brief Random number generator engine
    mutable std::mt19937 randomEngine{std::random_device{}()};

    /// @brief Initialize the synthesizer with the given topology and collective.
    /// @param topology target network topology
    /// @param collective target collective pattern
    /// @param chunkSize chunk size in bytes
    void initialize_(const std::shared_ptr<Topology> topology,
                     const std::shared_ptr<Collective> collective,
                     ChunkSize chunkSize) noexcept;
    ...
    /// @brief Merge persistent temp conditions into the current iteration map
    void mergeTempConditions_(PostconditionMap* postconditionMap) const noexcept;
    ...
    void addTempCondition_(ChunkID chunk, NpuID dest) noexcept;

    void removeTempCondition_(ChunkID chunk, NpuID dest) noexcept;

    [[nodiscard]] std::optional<RoutingHint> findRoutingHint_(ChunkID chunk,
                                                             NpuID dest) noexcept;

    [[nodiscard]] int computeConditionDistance_(ChunkID chunk, NpuID dest) noexcept;

    [[nodiscard]] bool isFinalDestination_(ChunkID chunk, NpuID dest) const noexcept;

    void removeAllTempConditionsForChunk_(ChunkID chunk) noexcept;

    /// @brief Simulate link-chunk matching for a given order without modifying real state
    /// @details Returns the number of links successfully utilized in this timestep
    /// @param postcondition ordered list of (chunk, dest) pairs to process
    /// @param availableCopy copy of current TEN availability state (modified during simulation)
    /// @param chunkMapCopy copy of current chunkMap_ state (modified during simulation)
    /// @param tempConditionsCopy copy of current tempConditions_ state (modified during simulation)
    /// @return number of links successfully allocated
    [[nodiscard]] int simulateLinkChunkMatching_(
    ...
    /// @brief Find routing hint considering both chunkMap_ and tempConditions_
    /// @details This version checks if chunk is in tempConditions_ as well
    /// @param chunk chunk ID
    /// @param dest destination NPU ID
    /// @param tempConditions optional temp conditions to consider (if null, uses member)
    /// @return routing hint if found
    [[nodiscard]] std::optional<RoutingHint> findRoutingHintWithTemp_(
        ChunkID chunk, NpuID dest, const PostconditionMap* tempConditions = nullptr) const noexcept;
