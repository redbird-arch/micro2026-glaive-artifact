/*
# File name  :    fattree.cpp
# Author     :    Galois
# Time       :    2026/01/22 16:21:02
*/
...
FatTree::FatTree(const std::vector<int>& shape,
                 int switchDimension,
    ...
    for (int nodeIdx = 0; nodeIdx < numNodes; ++nodeIdx) {
        ...
        // Each GPU connects to each switch with scaleUpLinksPerSwitch links
    ...
    if (switchDimension > 1) {
        ...
        // In fat-tree, each GPU connects to ALL first-layer scale-out switches
        // Each GPU connects to each first-layer switch
        for (int gpuID = 0; gpuID < numGPUs; ++gpuID) {
            for (int switchIdx = 0; switchIdx < scaleOutLayer1Count; ++switchIdx) {
                int switchID = scaleOutLayer1BaseID + switchIdx;
                // Connect GPU to each first-layer switch with multiple links
                int linksPerSwitch = scaleOutLayer1LinksPerGPU / scaleOutLayer1Count;
                for (int link = 0; link < linksPerSwitch; ++link) {
                    connect(gpuID, switchID, scaleOutLayer1Latency, scaleOutLayer1Bandwidth, true);
                }
            }
        }
        
        // Connect scale-out layers: each layer N switch connects to all layer N+1 switches
        // This is the fat-tree property: full connectivity between adjacent layers
...
std::string FatTree::formatNodeName(FatTree::NpuID id) const noexcept {
    int numGPUs = std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int>());
    int gpusPerNode = shape[0];
    int numNodes = (shape.size() >= 2) ? shape[1] : 1;
