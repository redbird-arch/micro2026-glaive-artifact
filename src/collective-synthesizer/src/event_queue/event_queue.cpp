...
void EventQueue::printEvents() noexcept {
    ThreadOutput::output("[Print EventQueue]: size = ");
    ThreadOutput::output(events_.size());
    ThreadOutput::output(", include: ");
    for (const auto& time : events_) {
